...
def update_github_repo(token, repo):
    """
...
def update_local_repo(repo):
    ...
    if not args.debug:
        ...
        if args.retire_project:
            gcpath = "%s/%s/config" % (REPO_ROOT, repo)
            if not os.path.exists(gcpath):
                gcpath = "%s/%s/.git/config" % (REPO_ROOT, repo)
        ...
        if os.path.isfile("%s/%s/description" % (REPO_ROOT, repo)):
            print("  - Updating description in %s/%s" % (REPO_ROOT, repo))
            archive_flag = " -- Archived"
            with open("%s/%s/description" % (REPO_ROOT, repo), 'a+') as desc:
                desc.write(archive_flag.rstrip('\n'))
                desc.close()
        # Set GitBox repo to read-only
        print("  - Setting Archive on GitBox")
        if not os.path.isfile("%s/%s/nocommit" % (REPO_ROOT, repo)):
            print("  - Creating %s/%s/nocommit" % (REPO_ROOT, repo))
...
actions.add_argument('-R', '--retire-project', action="store_true", help="Archive *ALL* repos belonging to project")
actions.add_argument('-A', '--archive-repo', action="store_true", help="Archive single repository by name")
